...
//std::unordered_set<int> Ransac(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	while(maxIterations--) {
		...
		for(int idx=0; idx<cloud->points.size(); idx++) {
			...
			float x4 = cloud->points[idx].x;
			float y4 = cloud->points[idx].y;
			float z4 = cloud->points[idx].z;
    ...
    for(const int& idx : inliersResult) {
        pcl_inliers->indices.push_back(idx);
    }

    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    std::cout << "plane segmentation using custom Ransac implementation took " << elapsedTime.count() << " milliseconds" << std::endl;
...
template<typename PointT>
void ProcessPointClouds<PointT>::ClusterHelper(int idx, const std::vector<std::vector<float>>& points, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol) {
	processed[idx] = true;
	cluster.push_back(idx);

	std::vector<int> near_pts = tree->search(points[idx], distanceTol);

	for(const int id : near_pts){
		if(!processed[id])
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::EucledianClustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    std::cout << "clustering using custom Eucledian Clustering took " << elapsedTime.count() << " milliseconds and found " << clusters.size() << " clusters" << std::endl;
